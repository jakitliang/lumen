...
LUA_API int lua_checkstack(Lumen::State *L, int size) {
    int res = 1;
    LumenLock(L);
...
LUA_API void lua_xmove(Lumen::State *from, Lumen::State *to, int n) {
    int i;
    if (from == to) return;
    LumenLock(to);
...
LUA_API void lua_setlevel(Lumen::State *from, Lumen::State *to) {
...
LUA_API int lua_pcall(Lumen::State *L, int nargs, int nResults, int errFunc) {
    ...
    c.NResults = nResults;
    status = Lumen::Do::PCall(L, &ProtectedCall::Call, &c, LumenSaveStack(L, c.Func), func);
    adjustResults(L, nResults);
    LumenUnlock(L);
    return status;
}
...
struct ProtectedCCall {
    ...
    static void Call(Lumen::State *L, void *ud) {
        ...
        apiIncrTop(L);
        LumenSetLUDataValue(L->Top, c->ud);  /* push only argument */
        apiIncrTop(L);
...
LUA_API int lua_cpcall(Lumen::State *L, Lumen::Delegate func, void *ud) {
    ...
    c.func = func;
    c.ud = ud;
    status = Lumen::Do::PCall(L, &ProtectedCCall::Call, &c, LumenSaveStack(L, L->Top), 0);
    LumenUnlock(L);
    return status;
}
...
LUA_API int lua_load(Lumen::State *L, Lumen::Reader reader, void *data,
    ...
    Lumen::ZIO::Init(L, &z, reader, data);
...
LUA_API int lua_dump(Lumen::State *L, Lumen::Writer writer, void *data) {
    ...
    if (LumenIsLFunction(o))
        status = Lumen::Dumper::Dump(L, LumenClosureValue(o)->AsLua.Func, writer, data, 0);
    else
        status = 1;
    LumenUnlock(L);
    return status;
