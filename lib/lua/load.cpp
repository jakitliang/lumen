/*!
 * @brief Dynamic library loader for Lua
...
LUAI_FUNC void ll_unloadlib(void *lib);

LUAI_FUNC void *ll_load(lua_State *L, const char *path);

LUAI_FUNC lua_CFunction ll_sym(lua_State *L, void *lib, const char *sym);
...
LPP_API int Lumen::Open<Lumen::IPackage>(Lumen::IState *L) {
    ...
    for (i = 0; loaders[i] != nullptr; i++) {
        L->PushFunction(loaders[i]);
        L->RawSetAt(-2, i + 1);
    }
    L->SetField(-2, "loaders");  /* put it in field `loaders' */
    setpath(reinterpret_cast<lua_State *>(L), "path", LUA_PATH, LUA_PATH_DEFAULT);  /* set field `path' */
    setpath(reinterpret_cast<lua_State *>(L), "cpath", LUA_CPATH, LUA_CPATH_DEFAULT); /* set field `cpath' */
    /* store config information */
    L->PushLiteral(LUA_DIRSEP "\n" LUA_PATHSEP "\n" LUA_PATH_MARK "\n"
                   LUA_EXECDIR "\n" LUA_IGMARK);
    L->SetField(-2, "config");
    /* set field `loaded` */
